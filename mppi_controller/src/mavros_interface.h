...
class Mavros_Interface {
    public:
        ...
        bool state_check() {
            if (ros::Time::now() - _state.header < ros::Duration(2)) {
                return (_state.has_armed && _state.offboard_enabled);
            } else {
                return false;
            }
        }
        ...
        template <class T>
        void pub_att_thrust_cmd(const Eigen::Quaternion<T> &q_d,const T& thrust_d) {
